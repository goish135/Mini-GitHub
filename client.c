...
int main(int argc, char *argv[]) {
    ...
    do {
        ...
        fprintf(stderr, "Please Type Request to server: \n[Usage]\ncreate filename permission\nread filename\nwrite filename o/a\nchangemode filename permission\n>>>");

        fgets(outputBuffer, BUFFER_SIZE, stdin); // 取得輸入
	...
	{
		printf("[INFO] Input is null\n");
		//outputBuffer = "Null Input";
		strcpy(outputBuffer,"Null Input12345678910\n");
	}
        outputBuffer[strlen(outputBuffer) - 1] = 0;
	/*
	// send/process file's context //
	int sockfd, n;
        ...
        int words = 0;
        char c;
        f=fopen("./client_side/glad.txt","r");
	...
	printf("The file was sent successfully");
        ...
        // End //
	*/
        if ( send(tcpSocketFileDescriptor, outputBuffer, strlen(outputBuffer) + 1, 0) == -1 ) { // send command 
