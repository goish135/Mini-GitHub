...
int acceptConnections(int tcpSocketFileDescriptor, int udpSocketFileDescriptor) {
    ...
    while ( TRUE ) {
        ...
        for ( i = 0; i < MAX_CONNECTIONS; ++ i ) {
			...
			{
				strcpy(inputBuffer,"Enjoy Read!");
				send(clientSocketFD, inputBuffer, strlen(inputBuffer), 0);
				status = 1; 
			}
			else if(status==2)
		        {
				printf("Read...");
				strcpy(inputBuffer,"Someone is Writing,you cannot read!!\n");
				send(clientSocketFD, inputBuffer, strlen(inputBuffer), 0);
			}		
		    }
		    else if(strcmp(inputBuffer,"Write")==0)
		    {
			    if(status==-1)
			    {
				 strcpy(inputBuffer,"Enjoy Write!");
				 send(clientSocketFD, inputBuffer, strlen(inputBuffer), 0);
				 status = 2;
			    }
			    else if(status==1)
			    {
				 strcpy(inputBuffer,"Someone is Reading,and you cannot write!!\n");
				 send(clientSocketFD, inputBuffer, strlen(inputBuffer), 0);
			    }
			    else if(status==2)
			    {
				 strcpy(inputBuffer,"Someone is Writing ,and you cannot write!!\n");
				  send(clientSocketFD, inputBuffer, strlen(inputBuffer), 0);
			    }
		    }
		    else  {
