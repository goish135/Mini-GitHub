...
if(status==1)
{
	...
	while(pch!=NULL)
	{
		...
		{
			printf("per:%s\n",pch);
			if(strcmp(pch,"o")==0) strcpy(ch,"w");
			if(strcmp(pch,"a")==0) strcpy(ch,"a");
			if(strlen(pch)==9)
			{
				printf("gopro\n");
				bzero(ch,sizeof(ch));
	...
	else if(strcmp(action,"Create")==0)
	{
		...
		bzero(buffer,sizeof(buffer));
		 ...
		 char dir[FILE_NAME_MAX_SIZE+1] = "./server_side/";  
		int len = 0;
		len = recv(new_server_socket,buffer,BUFFER_SIZE,0); // buffer -> context
		if(len==1) {printf("no context...\n");}
	...
	//	getdate(&dt);
		...
		printf("%s %s %d %d/%d/%d %s\n",ch,account,len,tm.tm_mon+1,tm.tm_mday,tm.tm_year+1900,file_name);
